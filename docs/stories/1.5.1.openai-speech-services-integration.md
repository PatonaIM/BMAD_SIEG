# Story 1.5.1: OpenAI Speech Services Integration (Backend)
...
- [ ] **Task 9: Add API Documentation** (AC: 1, 2)
  - [ ] Document `SpeechProvider` interface with docstrings:
    - Method signatures
    - Parameter descriptions
    - Return value specifications
    - Exception documentation
    - Usage examples
  - [ ] Document `OpenAISpeechProvider` class:
    - OpenAPI integration details
    - Rate limits and quotas
    - Audio format specifications
    - Cost information
  - [ ] Document configuration settings in `config.py`:
...
CREATE TABLE interview_messages (
    ...
    audio_metadata JSONB,                    -- ✅ For Whisper confidence, sample rate, etc.
...
│   ├── speech_provider.py              # 🆕 Abstract interface (CREATE)
│   ├── openai_speech_provider.py       # 🆕 OpenAI implementation (CREATE)
│   ├── base_ai_provider.py             # ✅ Exists from Story 1.4
│   └── openai_provider.py              # ✅ Exists from Story 1.4
├── services/                            # Business logic layer
│   ├── speech_service.py               # 🆕 Speech orchestration (CREATE)
│   ├── interview_engine.py             # ✅ Exists from Story 1.7
│   └── auth_service.py                 # ✅ Exists from Story 1.3
├── utils/                               # Utility functions
│   └── cost_calculator.py              # 🆕 Cost tracking (CREATE)
├── core/                                # Core configuration
│   ├── config.py                       # 📝 Add speech settings (MODIFY)
│   ├── exceptions.py                   # 📝 Add speech exceptions (MODIFY)
│   └── database.py                     # ✅ Exists from Story 1.2
└── schemas/                             # Pydantic models
    └── speech.py                       # 🆕 Speech request/response schemas (CREATE)
```
...
- **15 questions × 1 minute response** = 15 minutes STT = $0.09
- **15 questions × 200 chars each** = 3,000 chars TTS = $0.045
