...
1. Set up Supabase Storage bucket `interview-recordings`:
   ...
   - RLS policies: Not applicable (backend uses service_role key which bypasses RLS)
...
- [x] **Task 2: Configure Row Level Security (RLS) Policies** (AC: 1) - **N/A**
  - [x] **DECISION: Task marked as Not Applicable**
  - [x] **RATIONALE**: Backend uses Supabase `service_role` key which automatically bypasses all RLS policies
  - [x] **ARCHITECTURE**: All storage operations go through backend API (not direct frontend access)
  - [x] **SECURITY**: Authorization handled at application layer via API endpoints, not at storage layer
  - [x] **CONSEQUENCE**: RLS policies would be redundant and provide no additional security benefit
  - [x] **DOCUMENTATION**: RLS policy templates remain in `backend/docs/VIDEO_STORAGE.md` for future reference if architecture changes (e.g., direct frontend uploads)
  - [x] Original subtasks (preserved for reference):
    - Enable RLS on `interview-recordings` bucket
    - Create policy: `authenticated_upload` (for direct user uploads - not used)
    - Create policy: `org_member_read` (for direct user downloads - not used)
    - Create policy: `service_role_all` (service role already bypasses RLS by design)
    - Test RLS policies (not applicable without policies)
