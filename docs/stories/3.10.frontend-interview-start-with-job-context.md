...
- Depends on: Story 3.6 (Enhanced Interview Start Endpoint - Ready for Review ⚠️), Story 3.9 (Frontend Application Submission Flow - Draft 📝)
...
13. Role-specific preparation tips dynamically generated based on tech_stack (React → "Review component lifecycle", Python → "Brush up on list comprehensions", etc.)
...
- [ ] **Task 11: Test Backward Compatibility** (AC: 9)
  - [ ] Test interview start flow WITHOUT application_id:
    ...
    - [ ] Click button → verify interview starts with role_type="react"
...
- [ ] **Task 13: Manual Testing - Job-Linked Interview Flow** (AC: 1, 2, 3, 4, 5, 6, 7, 8, 10, 11, 12, 13, 14, 15)
  ...
  - [ ] **Test error handling - invalid application_id:**
    ...
    - [ ] Click button → verify navigation to `/applications`
  ...
  - [ ] **Test backward compatibility:**
    ...
    - [ ] Click button → verify interview starts and navigates to tech check
...
**From Story 3.9: Frontend Application Submission Flow (Draft 📝)**
...
**From Story 3.8: Frontend Dashboard & Applications Page Update (Ready for Manual Testing ⚠️)**
...
- [ ] Apply to job (Story 3.9) → get application_id
- [ ] Click "Start Interview Now" → navigate to interview start with application_id
- [ ] Click "Begin Interview" → verify interview created and linked to application
...
- [ ] Start interview → verify creates interview with role_type="react"
...
- [ ] Verify "View My Applications" button present
- [ ] Click button → verify navigates to `/applications`
...
- [ ] Login as Candidate A → apply to job → get application_id
- [ ] Logout → login as Candidate B
...
- [ ] Apply to job → start interview (TC2)
...
- [ ] Refresh page → verify job context loads correctly
...
- [ ] Click "Begin Interview" → verify mutation executes
...
9. **Linting**: Only one pre-existing style warning (`bg-gradient-to-br` → `bg-linear-to-br`) - left unchanged for consistency
...
**Dependencies:** Story 3.6 (Enhanced Interview Start Endpoint - Ready for Review ⚠️), Story 3.9 (Frontend Application Submission Flow - Draft 📝)  
